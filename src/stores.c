...
void stores_populate_store_items(store_s *stores, int store_count)
{
    ...
    for (int n = 0; n < store_count; n++)
    {
        ...
        {
        case COOP:
            test_fetch_get_store_items(&stores[n], n); // get_coop_items(&stores[n]); TODO: Implement when function get_coop_items has been implemented.
            break;
        case SALLING:
            test_fetch_get_store_items(&stores[n], n); // get_salling_items(&stores[n]); TODO: Implement when function get_salling_items has been implemented.
            break;
        default:
